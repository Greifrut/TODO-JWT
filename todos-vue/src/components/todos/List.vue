        ...
        <ul class="list-group">
            <li class="list-group-item" v-for="todo in todos" :key="todo.id" :todo="todo">
                ...
                <div v-show="todo == editedTodo">
                    <input class="form-control" v-todo-focus @blur="updateTodo(todo)" @keyup.enter="updateTodo(todo)" v-model="todo.title" />
                </div>
            </li>
        </ul>
    </div>
  ...
  created () {
    if (!localStorage.signedIn) {
      ...
      this.$http.secured.get('/todos')
        ...
        .cath(error => this.setError(error, 'Something went wrong'))
    }
  },
  methods: {
    setError (error, text) {
    ...
    addTodo () {
      ...
      this.$http.secured.post('/todos', {todo: {title: this.newTodo}})
        ...
        .cath(error => this.setError(error, 'Cannot create todo'))
    },
    removeTodo (todo) {
      this.$http.secured.delete(`/todos/${todo.id}`)
        ...
        .cath(error => this.setError(error, 'Cannot delete todo'))
    },
    editTodo (todo) {
      this.editTodo = todo
    },
    updateTodo (todo) {
      this.editedTodo = ''
      this.$http.secured.path(`/todos/${todo.id}`, {todo: { title: todo.title }})
        .cath(error => this.setError(error, 'Cannot updated todo'))
    }
  },
  directives: {
